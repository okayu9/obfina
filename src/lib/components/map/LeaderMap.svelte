<script lang="ts">
	...
	const nodes = $derived.by(() => {
		...
		const sim = forceSimulation(data)
			.force('x', forceX<N>((d) => d.x0).strength(0.5))
			.force('y', forceY<N>((d) => d.y0).strength(0.5))
			.force('collide', forceCollide<N>((d) => d.r + 1).strength(1))
			.stop();
...
<div class="map" bind:clientWidth={width} bind:clientHeight={height}>
	<svg bind:this={svgEl} {width} {height} role="presentation">
		<g {transform}>
			...
			<!-- Leader lines from each displaced bubble back to its true location -->
			{#each nodes as n (n.code)}
				{#if Math.hypot(n.x - n.x0, n.y - n.y0) > 1.5}
					<line class="leader" x1={n.x0} y1={n.y0} x2={n.x} y2={n.y} />
					<circle class="anchor" cx={n.x0} cy={n.y0} r="1.2" />
				{/if}
			{/each}
