<script lang="ts">
	...
	// Resolve each feature's ISO alpha-2 code once.
	...
	// Fill opacity encodes relay count (sqrt-ish via log for wide range).
	...
	const shapes = $derived.by(() => {
		...
		return countriesFc.features.map((f) => {
			...
			return {
				...
				d: path(f as never) ?? '',
				stats: stats ?? null,
				fill: stats ? roleColor(stats) : '#0d1825',
				opacity: stats ? 0.12 + intensity(Math.sqrt(stats.count)) * 0.85 : 1
			};
		});
	});
	...
	function onEnter(e: PointerEvent, code: string | null, stats: CountryStats | null) {
		...
		hover = { code, count: stats.count, x: e.clientX, y: e.clientY };
	}
